...
June AI：免费、单用户、打开即用的**无限追问 AI 辅助伴学系统**。用户 BYOK 连接自己的 AI 工具（AI 使用费用由用户自己的模型账户承担），通过主对话与**无限追问链**把一个想法一步步做成可用的成果。核心特色三条：①追问层级与次数都不限（线程树逐层生长）；②越问越聚焦的记忆（近层保留原文、远层自动压缩成结论、总量封顶不失忆）；③结论可一键采纳为交付物（聊过的每一步都沉淀为产出）。已完成 免费启动 → 伴学流程 → 完成归档 的完整闭环。**单用户模式（2026-09-19）**：登录系统与管理后台已整体移除，打开产品直达对话界面；所有 /api/* 请求由 `SingleUserMiddleware` 归属固定本地身份 `local`；支付链（2026-09-18）与用户体系相关代码已不存在，勿重新引入。

- 定位：本地单用户免费工具，**不是**生产上线版本（Docker 编排未实际运行过；支付链与登录系统均已整体移除）。
- 产品演变：AI 扫盲学堂 → 超级个体引导态 → Vibe Coding 引导态 → 登录门禁态 → **单用户伴学态（当前）**。**不要用早期定位理解当前产品。**
...
4. 课程/助手内容只在服务端，不提供任何导出/批量复制接口；不得向用户夸大防截图能力。
