...
June AI：面向零基础用户的「Vibe Coding 变现训练」**免费**产品。用户 BYOK 连接自己的 AI 工具，在 10 个必经节点引导下做出可售卖的最小商业 MVP。核心特色是**无限追问链**（层级/次数不限、逐层压缩 + 总量封顶、结论可采纳为交付物）。已完成 免费启动 → 训练流程 → 完成归档 的完整闭环。**单用户模式（2026-09-19）**：登录系统与管理后台已整体移除，所有 /api/* 请求由 `SingleUserMiddleware` 归属固定本地身份 `local`；支付链（2026-09-18）与用户体系相关代码已不存在，勿重新引入。
