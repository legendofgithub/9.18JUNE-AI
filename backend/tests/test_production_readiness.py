...
def test_stripe_checkout_and_signed_webhook(tmp_path, monkeypatch):
    monkeypatch.setattr(settings, "JUNE_PAYMENT_PROVIDER", "stripe")
    monkeypatch.setattr(settings, "JUNE_PUBLIC_BASE_URL", "https://june.example.com")
    monkeypatch.setattr(settings, "JUNE_STRIPE_SECRET_KEY", "sk_test_placeholder")
    monkeypatch.setattr(settings, "JUNE_STRIPE_WEBHOOK_SECRET", "whsec_test_placeholder")

    client, db, engine, _ = make_client(tmp_path)
    try:
        user = register_and_login(client, "stripe@example.com")

        async def fake_checkout(order, product):
            return {
                "id": f"cs_test_{order.id}",
                "url": f"https://checkout.stripe.com/c/pay/{order.id}",
            }

        client.app.state.commerce_service.payment.create_checkout = fake_checkout
        created = client.post("/api/orders", headers=auth_headers(user), json={
            "product_id": "super-solo-coach-unlock",
        })
        order = created.json()["data"]
        assert created.status_code == 200
        assert order["provider"] == "stripe"
        assert order["providerOrderId"].startswith("cs_test_")
        assert order["paymentUrl"].startswith("https://checkout.stripe.com/")
        ...
        manual_confirm = client.post(
            ...
            json={"provider_transaction_id": "pi_test_001"},
        )
        entitlement = client.get("/api/entitlements", headers=auth_headers(user)).json()["data"]
        payment_events = db.execute(text("SELECT COUNT(*) FROM payment_events")).scalar()

        assert webhook.status_code == 200
        assert webhook.json()["data"]["granted"] is True
        assert manual_confirm.status_code == 400
        assert entitlement["totalPaths"] == 1
