...
const copy = {
  zh: {
    title: '超级个体训练师',
    subtitle: '自动装载超级个体训练官 skill，启动 AI 跟踪交付流程。',
    paid: '已解锁',
    unpaid: '未解锁',
    installed: '人格已装载',
    notInstalled: '待装载',
  ...
  en: {
    ...
    paid: 'Unlocked',
    unpaid: 'Locked',
    installed: 'Persona loaded',
    notInstalled: 'Not loaded',
    keyReady: 'Key saved',
    keyMissing: 'Key required',
    start: 'Start Super-Solo Coach Persona',
...
export function CoachSettingsPanel({ language }: { language: SiteLanguage }) {
  const text = copy[language];
  const status = useCommerceStore(s => s.coachStatus);
  const isBusy = useCommerceStore(s => s.isBusy);
  const startCoach = useCommerceStore(s => s.startCoach);
  ...
  return (
    <details className="studio-settings-details">
      ...
      <div className="studio-settings-form">
        <div>
          <label className="coach-label" htmlFor="activeCoachModel">{text.tool}</label>
          <select id="activeCoachModel" className="coach-select" value={modelName} onChange={event => setModelName(event.target.value)}>
            {AI_TOOL_OPTIONS.map(option => (
              <option key={option.model} value={option.model}>{option.label}</option>
            ))}
          </select>
        ...
        <div>
          <label className="coach-label" htmlFor="activeCoachApiKey">{text.apiKey}</label>
          <input
            id="activeCoachApiKey"
            className="coach-input"
            type="password"
            value={apiKey}
            onChange={event => setApiKey(event.target.value)}
        ...
        <button
          ...
          disabled={isBusy || (!status?.apiKeyReady && !apiKey.trim())}
          onClick={() => void startCoach(modelName, baseUrl, apiKey.trim())}
        >
